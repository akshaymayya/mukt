...
export default function App() {
  ...
  return (
    <>
      ...
      {showWaitlist && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(5px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ background: '#fff', padding: '24px', borderRadius: '24px', width: '100%', maxWidth: '450px', position: 'relative' }}>
            <button onClick={() => setShowWaitlist(false)} style={{ position: 'absolute', top: '16px', right: '16px', fontSize: '1.5rem', color: '#666' }}>&times;</button>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Ravi Kumar" style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', border: '1px solid #e2e8f0', outline: 'none' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: 600, marginBottom: '8px' }}>Email</label>
                <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ravi@example.com" style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', border: '1px solid #e2e8f0', outline: 'none' }} />
              </div>
              ...
              {status.message && (
                <p style={{ color: status.error ? '#ef4444' : '#00d094', textAlign: 'center', marginTop: '10px', fontWeight: 500, fontSize: '0.9rem' }}>
                  {status.message}
                </p>
              )}
            </form>
      ...
      <nav className="navbar">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setShowWaitlist(true)} className="btn btn-primary" style={{ padding: '10px 20px', fontSize: '0.9rem' }}>
            Get Early Access
          </button>
        </div>
      </nav>
      ...
      <div className="grid grid-cols-2 gap-8 md:grid-cols-1">
        ...
        >
          <div>
            ...
            {expanded === 'map' && (
              <motion.div
                ...
                style={{ paddingTop: '16px', borderTop: '1px solid var(--border-light)' }}
                className="mb-6"
              >
                <div className="flex-col gap-3">
                  <div className="flex gap-2">
                    <CheckCircle2 className="text-accent-primary" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>Auto-sync:</strong> securely pull balances via Account Aggregator.</span>
                  </div>
                  <div className="flex gap-2">
                    <CheckCircle2 className="text-accent-primary" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>True Cost Calculator:</strong> Uncovers hidden processing fees & compound math.</span>
                  </div>
                  <div className="flex gap-2">
                    <CheckCircle2 className="text-accent-primary" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>Unified Dashboard:</strong> No more logging into 5 different bank apps.</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <button
            ...
            className="flex items-center gap-2 text-accent-dark font-bold hover:text-accent-primary transition-colors cursor-pointer"
            style={{ width: 'fit-content', marginTop: 'auto' }}
          >
            {expanded === 'map' ? 'Show less' : 'Learn more'}
            <ChevronRight size={16} style={{ transform: expanded === 'map' ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }} />
          </button>
        </motion.div>

        <motion.div
          layout
          whileHover={{ y: expanded === 'strategy' ? 0 : -8 }}
          className="p-card flex flex-col justify-between"
          style={{ background: 'linear-gradient(145deg, rgba(255,255,255,0.9), rgba(255,255,255,0.4))' }}
        >
          <div>
            ...
            {expanded === 'strategy' && (
              ...
              >
                <div className="flex-col gap-3">
                  <div className="flex gap-2">
                    <Target className="text-blue-600" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>Avalanche Mode:</strong> Math-first approach targeting highest APR to save maximum money.</span>
                  </div>
                  <div className="flex gap-2">
                    <Target className="text-blue-600" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>Snowball Mode:</strong> Psychology-first approach targeting smallest balances for quick wins.</span>
                  </div>
                  <div className="flex gap-2">
                    <Target className="text-blue-600" size={16} style={{ marginTop: '4px', flexShrink: 0 }} />
                    <span><strong>Dynamic Adjustments:</strong> If you miss a payment, the engine seamlessly recalibrates your entire plan.</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <button
            ...
            className="flex items-center gap-2 text-accent-dark font-bold hover:text-blue-600 transition-colors cursor-pointer"
            style={{ width: 'fit-content', marginTop: 'auto' }}
          >
            {expanded === 'strategy' ? 'Show less' : 'Learn more'}
            <ChevronRight size={16} style={{ transform: expanded === 'strategy' ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }} />
          </button>
        </motion.div>
