...
export default function DashboardApp({ onBack }) {
  ...
  const handleGlobalClick = (e) => {
    const isClickable = e.target.closest('button') || e.target.closest('.nav-item') || e.target.closest('.cursor-pointer') || e.target.closest('a') || e.target.closest('.glass-panel') ;
    if (isClickable) {
      setClickCount(prev => {
  ...
  return (
    <div className="flex dashboard-theme-wrapper" onClick={handleGlobalClick}>
      {/* Sidebar */}
      ...
      {showGoodTimes && (
        <div className="overlay" style={{ zIndex: 99999 }}>
          <div className="modal animate-fade-in text-center">
            <h2 className="text-3xl font-bold mb-4 font-['Outfit'] text-white">Hold on tight!</h2>
...
function DebtMap({ debtData, totalDebt }) {
  return (
    <div className="animate-fade-in delay-100 flex gap-8 debt-map-layout">
          ...
          {debtData.map((debt, index) => {
            ...
            return (
              <div key={debt.id} className="loan-item glass-panel" style={{ padding: '16px', boxShadow: 'none' }}>
                <div className="h-12 w-12 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: `${debt.color}20`, color: debt.color }}>
                  <Icon size={24} />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-lg truncate">{debt.name}</h4>
                ...
                <button className="btn btn-secondary !p-2 !rounded-lg shrink-0 hidden md:block"><ChevronRight size={20} /></button>
              </div>
            )
          })}
        </div>
      ...
      <div className="flex flex-col gap-6 mx-auto" style={{ width: '100%', maxWidth: '400px' }}>
  ...
  return (
    <div className="animate-fade-in delay-100 max-w-4xl mx-auto">
      ...
      <div className="glass-panel p-8 text-center bg-gradient-to-br from-bg-glass to-accent-primary/5">
        ...
        <div className="bg-black/40 rounded-xl p-6 md:p-6 p-4 inline-block text-left border border-white/10 mx-auto w-full max-w-[350px]">
